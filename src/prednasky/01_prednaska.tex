...
\begin{theorem}[NT and DS relation]
...
M pracuje v case $g(n) = O(f(n))$, vypocet v 4. skonci do g(n) kroku.

Prostor: \\
1) na ulozeni k potrebujeme g(n) prostoru, $ k \leq g(n) $. \\
2) Na simulace M(x) dle y potrebujeme O(g(n)) prostoru.
...
\begin{proof}
	...
	How to avoid double exponent? $\Rightarrow$ Perform BFH in conf. graph.
...
\begin{enumerate}
	...
	\item foreach $x\triangle^i, i = 0, 1, ... $ \\
...
\begin{proof}
	...
	\[ S_1 = \frac{s+1}{t}, S_2 = \frac{s}{t}, f(n) = n^{(s + i)t}, i = 0, 1 ..., t \]
	We get
