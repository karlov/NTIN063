...
\subsection{Polynomial hierarchy again}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item
			\[ \Delta_k \xlongequal{a)} co-\Delta_k = co-\TP(\Sigma_{k - 1}) = co-\TP(\Pi_{k - 1}) \subseteq co-\TNP(\Pi_{k - 1}) = \Pi_k \]
	\end{enumerate}
...
\begin{properties}[Polynomial Hierarchy with quantifiers].
	\begin{enumerate}[label=\alph*)]
		\item $\exists \TP = \TNP$
